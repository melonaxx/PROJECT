$(function(){
	...
	function getParam(){
		...
		if ($('.pmethodid').val())
		{
			var pmethodid = $('.pmethodid').val();
		} else {
			var pmethodid = '';
		}
		paramarr['pmethodid'] = $.trim(pmethodid);
	...
	function keepPageSearch(search)
	{
		...
		var pmethodid   = getParam().pmethodid;
		var pagesize = getParam().pagesize;
		var url = location.href.substring(0,location.href.indexOf('?'));
		window.location.href=url+"?page="+page+"&search="+search+"&pmethodid="+pmethodid+"&pagesize="+pagesize;
	}
	/*切换页大小*/
	...
	/*保持印刷方式选中状态*/
	if (GetRequest().pmethodid)
	{
		$.each($('.pmethodid option'),function(i){
			if ($('.pmethodid option').eq(i).val() == GetRequest().pmethodid)
			{
				$('.pmethodid option').eq(i).attr('selected',true);
			}
		});
	}
