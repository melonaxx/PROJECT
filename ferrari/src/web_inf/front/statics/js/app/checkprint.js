$(function(){
	...
	$(".sure-check").on("click",function(){
		...
		/*印刷单审核确认btn*/
		$('.checkpbillbtn').off('click').click(function(){
			$(".modal-check").hide();
			var checkpbsuccess = function(msg)
			{
				location.reload();
			}
			var checkpbfail = function()
			{
				alert('提交异常失败！');
				location.reload();
			}
			util.ajax_post('/app/checkprintbill.php',{pbillid:pbillid},checkpbsuccess,checkpbfail);
		});
	});
})