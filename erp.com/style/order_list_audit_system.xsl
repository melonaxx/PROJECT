...
<script type="text/javascript">
	...
	<!-- 订单的审核 -->
		$('.shenhe').click(function(){
			if($(".tab_select input[type='checkbox']").is(':checked')){
				...
				$('.ok').click(function(){
					...
					if(a){
						$.ajax({
							url:'order_list_audit_system.php',
							type:'get',
							dataType:'json',
							...
							success:function(data){
								if(isArray(data)){
									...
									<!-- $('#con').modal('hide'); -->
									window.location.href="order_list_audit_system.php";
									return false;
								}else if(data == '0'){
									alert('修改失败！');
									return false;
		...
		$('.he').click(function(){
			...
			MessageBox('/order/order_list_audit_detach.php?id='+a, '手工合单',890,220);
			return false;
		});
...
<div class="mainBody">
	<ul id="myTab" class="nav nav-tabs" style="margin-bottom:10px;">
	   <li><a href="order_list_audit.php">线上订单</a></li>
	   <li class="active"><a href="order_list_audit_system.php">线下订单</a></li>
	</ul>
		...
		<div class="goodsMsg" style="clear:both;">
			...
			<table style="width:1200px;" class="table tab_select table-bordered table-hover table-order-form">
				<tr>
					...
					<th class="table_th_number" style="width:98px;">标记</th>
