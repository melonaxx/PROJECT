...
<div class="mainBody">
	<ul id="myTab" class="nav nav-tabs">
		<li class="tab-pane active"><a href="/product/product_specifications_properties.php">规格管理</a></li>
...
<div class="mainBodyBody">
	...
	<div id="myStore" class="sasl">
		<div class='top'>
			<span class='top_span' >规格</span>
			<span class="top_button" type="button" onclick="MessageBox('product_add_format_name.php', '添加规格',240, 75)">添加新规格</span>
		</div>
		<div>
			<ul  id="bbb">
				...
				<li>
					...
					<div class='mySto2'>
						...
						<xsl:element name="SPAN">
							...
							<xsl:attribute name="onclick">MessageBox('/product/product_edit_format_name.php?id=<xsl:value-of select="id" />', '修改规格', 300, 150)</xsl:attribute>
							<img src="/images/iconfont-mod.svg" width='16px;' height='32px;' />
						</xsl:element>
						<xsl:element name="SPAN">
							<xsl:attribute name="class">myDel</xsl:attribute>
							<xsl:attribute name="onclick">MessageBox('/product/product_delete_format_name.php?id=<xsl:value-of select="id" />', '删除规格', 300, 150)</xsl:attribute>
	...
	<div id="myArea" class="sasl">
		<div class='top'>
			<span class='top_span' >规格值</span>
			<xsl:element name="SPAN">
				<xsl:attribute name="type">button</xsl:attribute>
				<xsl:attribute name="class">top_button</xsl:attribute>
				<xsl:attribute name="onclick">MessageBox('product_add_format_name.php','添加规格', 240, 75); return false</xsl:attribute>
...
<script>
	function mmm(id){
		...
		$.ajax({
			url:'/product/product_view_format_name.php',
			type:'post',
			data:{'id':store_id},
			dataType:'json',
	...
	function Area_S(aa){
		var area_id = aa;
		MessageBox('/product/product_edit_format_name.php?id='+area_id,'修改规格',300,150);
	}

	//规格值删除
	function Area_SS(aa){
		var area_id = aa;
		MessageBox('/product/product_delete_format_name.php?id='+area_id,'删除',300,150);
	}

	//规格值修改
	function Area_M(aa){
		var area_id = aa;
		MessageBox('/product/product_edit_format_value.php?id='+area_id,'修改规格值',300,150);
	}

	//规格值值删除
	function Area_D(aa){
		var area_id = aa;
		MessageBox('/product/product_delete_format_value.php?id='+area_id,'删除',300,150);
	}

	//规格值添加
	$('#myArea .top .top_button').on('click',function(){
		var stock_id = $('input[name=stock_id]').val();
		MessageBox("product_add_format_value.php?format_id="+stock_id, '添加', 300, 150);
	})
