	...
	$(function(){
		$('.dz').click(function(){
			if($(".sure input[type='checkbox']").is(':checked')){
				$('#confirm .modal-body').html("您要确认定制吗?");
				$('#confirm').modal('show');
				$('.ok').click(function(){
					var arr = new Array();
					...
					$.ajax({
						url:'order_custom.php',
						type:'get',
						data:{'audit':arr},
						dataType:'json',
						success:function(data){
							if(data == '1'){
								window.location.href ="order_custom.php";
							}else{
								alert('修改失败');
							}
						}
...
<div class="mainBody">
	<ul class="nav nav-tabs nav_tabs_margin">
		<li class="active"><a href="order_custom.php" >待确认</a></li>
		<li><a href="order_custom_r.php" >待定制</a></li>
		<li><a href="order_custom_y.php" >已完成</a></li>
	</ul>
	<div class="tab-content">
		<div class="tab-pane fade in active" id="wait_confirm">
			<div class="form-inline">
				<div class="waitConfirmMsg">
					<form action="order_custom.php" method="get">
					<div class="table_operate_block">
						...
						<button class="btn btn-default btn-sm dz" type="button">确认定制</button>
					</div>
					</form>
					<table style="width:1200px" class="table table-bordered table-hover sure">
						<tr>
