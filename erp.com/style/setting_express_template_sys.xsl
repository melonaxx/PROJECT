...
<div class="mainBody">
	<div class="headMsg table_operate_block">
		<form action="/setting/setting_express_template_sys.php" method="post" class="form-inline">
		...
		<a href="/setting/setting_express_template.php"><input class="btn btn-default btn-sm btn_margin" type="button" value="我的模板" /></a>
	...
	<table style="" class="table tab_select table-bordered table-hover">
		<tr>
			...
			<th style="width:376px;text-align:center" rowspan="2">模板图片</th>
		...
		<tr>
			...
			<td class="center">
				...
				<!-- <xsl:element name="A">
					...
					<xsl:attribute name="href">/setting/setting_express_template_sys.php?id=<xsl:value-of select = 'id'/></xsl:attribute>
					<xsl:attribute name="is_default"><xsl:value-of select = 'is_default'/></xsl:attribute>
				<xsl:text>添加到我的模板</xsl:text>
				</xsl:element> -->


				<form action="/setting/setting_express_template_sys.php?action=add" method="get" >
					<div class="input-group">
						<xsl:element name="INPUT">
	...
	$('.table .delete').click(function () {
		...
		$('#confirm .ok').click(function () {
			$.ajax({
				url: thisHref,
				type: 'get',
				data: {action:'add',template_name:$('#template_name').val()},
				success: function(data){
					if(data!=0){
						alert('添加成功！');
					}
					$('#confirm').modal('hide');
					location.href = '/deliver/deliver_design_express.php?template_id='+data;
				}
			});
		});
		return false;
