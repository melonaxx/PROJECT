...
<script type="text/javascript" src="/js_encode/order_list_audit_detail.js"></script>
<!-- 时间插件 -->
...
<script type="text/javascript">
	...
	var is_audit= '<xsl:value-of select="/html/Body/supplierMsg/type/ul/li/is_audit"/>';
</script>
<script>
	$(function(){
		<!-- 获取购买渠道的信息当是线上的订单时客户留言是不能变化的 -->
		var bind_type = $('.supplierMsg select[name=bind_type] option:selected').val();
		if(bind_type != 'System') {
			$('.supplierMsg input[name=order_text]').attr('readonly',true);
		}
	})
</script>
...
<div class="mainBody">
<form class="form-inline margin_top_add2" action="" method="post">
	<div class="orderMsg">
		   ...
		   <div class="form-group">
			...
			<div class="form-group">
				...
				<xsl:element name="INPUT">
					...
					<xsl:attribute name="class">form-control input-sm</xsl:attribute>
					<xsl:attribute name="value"><xsl:value-of select="nick_name"/></xsl:attribute>
				</xsl:element>
			</div>
			<div class="form-group">
				<label>购买渠道：</label>
			...
			<div class="form-group">
				<label>店铺名称：</label>
				<select class="form-control input-sm shop" name="shop_title">
			...
			<div class="form-group">
				...
				<label>到账账户：</label>
				<select class="form-control input-sm bank" name="bank_id">
		...
		<div class="goodsMsg">
			...
			<table style="margin-top:10px;width:1200px;" class="table table-bordered table-hover">
				...
				<tr>
					...
					<td class="order_add_td_2" style="width:200px;">
						<select class="form-control input-sm form_no_border find" name="good_name[]">
							<xsl:element name="OPTION">
					...
					<td class="order_add_td_2" style="width:80px;">
						...
						<div class="input-group">
						  <div class="input-group-addon danjia">￥<xsl:value-of select='price' /></div>
						  	<xsl:element name="INPUT">
				...
				<tr class="total">
					...
					<td class="order_add_td_2" style="width:80px;">
						<div class="input-group">
						  <div class="input-group-addon zong">￥<xsl:value-of select='zong'/></div>
						  <xsl:element name="INPUT">
								<xsl:attribute name="type">hidden</xsl:attribute>
								<xsl:attribute name="name">total_pay</xsl:attribute>
								<xsl:attribute name="class">total_pay</xsl:attribute>
								<xsl:attribute name="value"><xsl:value-of select='zong'/></xsl:attribute>
							</xsl:element>
						</div>
		...
		<div class="commonMsg totalMsg">
			<p>商品总金额
				...
				<input class="form-control input-sm" style="width:69px;margin:0 4px;" readonly="readonly" type="text"/>元 + 运费
				<xsl:element name="INPUT">
					<xsl:attribute name="type">text</xsl:attribute>
					<xsl:attribute name="class">form-control input-sm</xsl:attribute>
				...
				<select class="form-control input-sm audit" name="is_audit" style="width:147px;">
			...
			<div class="form-group form-group_bottom0">
				...
				<div class="input-group">
				   ...
				   <xsl:element name="INPUT">
					...
					<input type="hidden" name="beforeAmount" value=''/>
