...
<script>
	$(function(){
		$('.su').click(function(){
			if($(".sure input[type='checkbox']").is(':checked')){
				$('#confirm .modal-body').html("您要确认完成定制吗?");
				$('#confirm').modal('show');
				$('.ok').click(function(){
					var arr = [];
					$(".sure input[name='select_one']:checked").each(function(){
						var v = $(this).next().val();
						arr += v+",";
					})
					$.ajax({
						url:'order_custom_r.php',
						type:'get',
						data:{'id':arr},
						dataType:'json',
						success:function(data){
							if(data == 1){
								window.location.href ="order_custom_r.php";
							}else{
								alert('修改失败');
							}
						}
		...
		$('.dahui').click(function(){
			if($(".sure input[type='checkbox']").is(':checked')){
				$('#confirm .modal-body').html("您要打回确认吗?");
				$('#confirm').modal('show');
				$('.ok').click(function(){
					var arr = [];
					$(".sure input[name='select_one']:checked").each(function(){
	...
	<div id="confirm" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" style="display: none;width:268px;margin:65px auto">
		<div class="modal-content">
			...
			<div class="modal-body" style="margin-left:20px">请至少选择<span class="body">1</span>个订单
			</div>
...
<div class="mainBody">
	<ul class="nav nav-tabs nav_tabs_margin">
		...
		<li class="active"><a href="order_custom_r.php">待定制</a></li>
		<li><a href="order_custom_y.php">已完成</a></li>
	</ul>
		<div class="tab-pane" id="wait_customize">
	   		<div class="form-inline">
				<div class="waitCustomizeMsg">
					<div class="table_operate_block">
						<form class="form-inline" method="get" action="order_custom_r.php">
						...
						<button class="btn btn-default btn-sm su" type="button">定制完成</button>

						<button style="margin-left:15px"  class="btn btn-default btn-sm dahui" type="button">打回确认</button>
					</div>
					<table style="width:1200px" class="table sure table-bordered table-hover">
						<tr>
						...
						<xsl:for-each select="/html/Body/dai/ul/li">
						<tr>
				...
				<xsl:if test="/html/Body/dai/ul/@total = '0'">
