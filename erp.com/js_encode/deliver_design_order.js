$(function(){
	...
	function init()
	{
		...
		var page_width  = $("#page_width").val() && !isNaN($("#page_width").val())?$("#page_width").val():210;
		var page_height = $("#page_height").val() && !isNaN($("#page_height").val())?$("#page_height").val():300;
		var page_top 	= $("#page_top").val() && !isNaN($("#page_top").val())?$("#page_top").val():0;
		var page_left 	= $("#page_left").val() && !isNaN($("#page_left").val())?$("#page_left").val():0;
		var print_orient  = $("#print_orient").val();
		LODOP.PRINT_INITA(page_top+"mm",page_left+"mm",page_width+"mm",page_height+"mm","");
		LODOP.SET_SHOW_MODE("DESIGN_IN_BROWSE",1);
		var page_name = $('#page_name').val();
		// LODOP.SET_PRINT_PAGESIZE(print_orient,page_width+"mm",page_height+"mm",page_name);
		LODOP.SET_PRINT_PAGESIZE(print_orient,'','',page_name);
		...
		$('.order_item').each(function(){
			if($(this).attr('id_exist')){
				...
				var font_bold 	= $(this).attr('item_font_bold')?$(this).attr('item_font_bold'):0;
				var type 		= $(this).attr('item_type')?$(this).attr('item_type'):'other';
				var item_id 	= this.id;
				var english 	= $(this).attr('english');

				LODOP.ADD_PRINT_TEXTA(type+"***"+item_id+"***"+english,top,left,width,height,this.value);
	...
	function Moditify(item,top,left,width,height,font_size,font_bold,type,item_id,english,content){
		...
		content 	= content != "" ? content : item.value;
	...
	$("#right_sign").click(function(){
		LODOP = getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'));
		LODOP.ADD_PRINT_TEXTA('defined******right-sign',30,30,30,30,'√');
		LODOP.SET_PRINT_STYLEA(0,"FontSize",14);
		LODOP.SET_PRINT_STYLEA(0,"Bold",1);
	});

	$("#word_text").click(function(){
		LODOP = getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'));
		LODOP.ADD_PRINT_TEXTA('defined******word-text',30,30,100,20,'自定义文本');
		LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
		LODOP.SET_PRINT_STYLEA(0,"Bold",0);
	});

	$("#transverse_line").click(function(){
		LODOP = getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'));
		LODOP.ADD_PRINT_LINE(41,83,40,362,0,1);
		LODOP.SET_PRINT_STYLEA(0,"FontSize",14);
		LODOP.SET_PRINT_STYLEA(0,"Bold",1);
	});
	...
	$("#vertical_line").click(function(){
		LODOP = getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'));
		LODOP.ADD_PRINT_LINE(227,83,20,84,0,1);
		LODOP.SET_PRINT_STYLEA(0,"FontSize",14);
		LODOP.SET_PRINT_STYLEA(0,"Bold",1);
	});
	...
	$("#preview").click(function(){
		...
		for(var i=0;i<count;i++){
			...
			template_position[i]['ItemClass']   = LODOP.GET_VALUE("ItemClass",i+1);
			// template_position[i]['ItemClassName']   = LODOP.GET_VALUE("ItemClassName",i+1);
		...
		// 设置纸张大小 和 类型
		var page_width  = $("#page_width").val() && !isNaN($("#page_width").val())?$("#page_width").val():210;
		var page_height = $("#page_height").val() && !isNaN($("#page_height").val())?$("#page_height").val():297;
			
		if(page_width<20 || page_width>400 || page_height<10 || page_height>400){
			alert("请设置合适的纸张大小");
			return false;
		}
		LODOP.SET_PRINT_PAGESIZE(print_orient,page_width+'mm',page_height+'mm',"");
		// console.log(template_position);return false;
		// 边框设置
		...
		for(var i = 1; i <= print_total_num; i++){
			...
			for(var k in template_position){
				...
				else if(template_position[k]['type'] == 'title'){
				...
				else if(template_position[k]['type'] == 'other'){
				...
				// 商品列表
				else if(template_position[k]['type'] == 'table'){
					table[n] = k;
					n++;
				}
				
				else if(template_position[k]['ItemClass']=='3'){
					LODOP.ADD_PRINT_LINE(template_position[k]['top'],template_position[k]['left'],template_position[k]['width'],template_position[k]['height'],0,1);
					LODOP.SET_PRINT_STYLEA(0,"FontSize",14);
					LODOP.SET_PRINT_STYLEA(0,"Bold",1);
				}
	...
	$('#add_template').click(function(){
		get_info();
		$.ajax({
			url: '/deliver/deliver_add_order_template.php',
			type: 'POST',
			dataType: 'text',
		...
		.done(function(data) {
			if(data == "ok"){
				alert("保存成功！");
				location.href = "/setting/setting_order_template.php";
			}else if(data == "exit"){
				alert("模板名称已存在！请重新命名！");
			}else{
				alert("新模板名称不能为空！");
	...
	$('#edit_template').click(function(){
		get_info();	
		$.ajax({
			url: '/deliver/deliver_edit_order_template.php',
			type: 'POST',
			dataType: 'text',
	...
	function get_info(){
		...
		// 模板信息
		template_info 		 	= "";
		template_info 		 	= "{'template_id':'"+$("#template_id").attr('value')+"','template_name':'"+$("#template_name").val()+"','template_image':'"+$("#template_image").attr('value')+"','border':'"+$("input[type=radio]:checked").val()+"','page_width':'"+page_width+"','page_height':'"+page_height+"','page_top':'"+$("#page_top").val()+"','page_left':'"+$("#page_left").val()+"','print_orient':'"+$("#print_orient").val()+"'}"
		// 获取每个打印项的信息
		var count 			 	= LODOP.GET_VALUE('ItemCount',0);
		// 控件位置信息
		template_position 		= "[";
 ...
 //       	}	
	...
	// 	}
	// 	$.ajax({
	// 		url: '/deliver/deliver_design_order.php',
	// 		type: "POST",
 ...
 //            cache: false,
	...
	// 选择打印模板
	$("body").on('click','#order_template',function(){
		...
		td.find("select option").click(function(){
			...
			location.replace("/deliver/deliver_design_order.php?template_id="+template_id);
		});
    });
	...
	$("#page_name").change(function(){
		...
		switch(page_name){
			case "0":$("#page_width").val("");$("#page_height").val("");break;
			case "A0":$("#page_width").val("841");$("#page_height").val("1189");break;
			case "A1":$("#page_width").val("594");$("#page_height").val("841");break;
			case "A2":$("#page_width").val("420");$("#page_height").val("594");break;
			case "A3":$("#page_width").val("297");$("#page_height").val("420");break;
			case "A4":$("#page_width").val("210");$("#page_height").val("297");break;
			case "A5":$("#page_width").val("148");$("#page_height").val("210");break;
			case "A6":$("#page_width").val("105");$("#page_height").val("127");break;
			case "B4":$("#page_width").val("257");$("#page_height").val("364");break;
			case "B5":$("#page_width").val("182");$("#page_height").val("257");break;
			case "8K":$("#page_width").val("368");$("#page_height").val("260");break;
			case "16K":$("#page_width").val("260");$("#page_height").val("184");break;
			default:$("#page_width").val("");$("#page_height").val("");
		}
		init();
	});
