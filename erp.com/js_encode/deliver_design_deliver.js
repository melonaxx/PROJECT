$(function(){
	...
	function init(){	
		...
		// LODOP.SET_SHOW_MODE("DESIGN_ENABLESS",10010111000001);//隐藏打印维护窗口
		$('.deliver_item').each(function(){
	...
	$('#add_template').click(function(){
		get_info();
		$.ajax({
			url: '/deliver/deliver_add_deliver_template.php',
			type: 'POST',
			dataType: 'text',
		...
		.done(function(data) {
			if(data == "ok"){
				alert("保存成功！");
				location.href = "/setting/setting_deliver_template.php";
			}else if(data == "exit"){
				alert("模板名称已存在！请重新命名！");
			}else{
				alert("新加模板名称不能为空！");
	...
	$('#edit_template').click(function(){
		get_info();	
		$.ajax({
			url: '/deliver/deliver_edit_deliver_template.php',
			type: 'POST',
			dataType: 'text',
	...
	function get_info(){
		LODOP = getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'));
		var page_width  = $("#page_width").val() && !isNaN($("#page_width").val())?$("#page_width").val():229;
		var page_height = $("#page_height").val() && !isNaN($("#page_height").val())?$("#page_height").val():126;
 ...
 //       	}	
	...
	// 	}
	// 	$.ajax({
	// 		url: '/deliver/deliver_design_deliver.php',
	// 		type: "POST",
 ...
 //            cache: false,
	...
	$("body").on('click','#deliver_template',function(){
		...
		td.find("select option").click(function(){
			...
			location.replace("/deliver/deliver_design_deliver.php?template_id="+template_id);
		});
    });
