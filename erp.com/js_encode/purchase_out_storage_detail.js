$(function () {
	...
	$(document).on('keyup','input[name="sum[]"]',function(){
		...
		var out_sum2_old = t.parents('tr').find('input[name="out_sum2[]"]').val().replace(/[^0-9]/ig,"");
		if(out_sum2_old == ""){
			out_sum2_old = '0';
		...
		if(parseInt(sum_x) > parseInt(in_sum_old)){
			sum_x = in_sum_old;
			t.val(sum_x);
		}

		in_now = parseInt(in_sum_old) - parseInt(sum_x);
		out_now = parseInt(out_sum_old) + parseInt(sum_x);

		// //新的出入库数量
		// out_sum2_old = sum_x;
		// in_sum2_old = sum_x;

		// var out_sum_x = parseInt(out_sum2_old)+parseInt(out_sum_old);
		// if(parseInt(out_sum_x)>parseInt(in_sum_old)){
		// 	out_sum_x = in_sum_old;
		// 	sum_x = parseInt(in_sum_old)-parseInt(out_sum2_old)
		// }
		// var in_sum_x = parseInt(in_sum_old)-parseInt(in_sum2_old);
		// if(parseInt(in_sum_x)<1){
		// 	in_sum_x = 0;
		// }
		// //新的在途数量
		// var zt_sum_x = parseInt(total)-parseInt(out_sum_x)-parseInt(in_sum_x);
		...
		t.parents('tr').find('td').eq(9).find('span').text(in_now);
		t.parents('tr').find('input[name="out_sum2[]"]').val(sum_x);
		t.parents('tr').find('td').eq(11).find('span').text(out_now);
	...
	$('textarea[name=brief]').blur(function(){
		...
		$.ajax({
			url:'purchase_out_storage_detail.php',
			type:'post',
