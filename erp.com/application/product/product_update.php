...
if(!empty($_GET['num_iid'])){
	$num_iid = replace_safe($_GET['num_iid']);
}

if(!empty($_GET['format_list'])){
	$format_list = replace_safe($_GET['format_list']);
}
					...
					if($total == '0'){
						...
						if($id){
							...
							while($res = mysql_fetch_object($this1)){
								$sql = "INSERT INTO store_product SET company_id='$company_id', store_id='$res->store_id',product_id='$id'";
								mysql_query($sql,$_mysql_link_);
								$sql = "INSERT INTO store_related SET company_id='$company_id', store_id='$res->store_id',product_id='$id'";
								mysql_query($sql,$_mysql_link_);
							}
						}
					}
...
// $params			= array();
// //获取单个商品详细信息
// $params['method']	= 'taobao.item.seller.get';
// $params['fields']	= 'num_iid, title, nick, sku.properties_name, sku.price, sku.quantity, pic_url, stuff_status,approve_status';
...
// $num_iid = replace_safe($json_data->item_seller_get_response->item->num_iid);
// $pic_url = replace_safe($json_data->item_seller_get_response->item->pic_url);
// $title   = replace_safe($json_data->item_seller_get_response->item->title);
// $stuff_status = replace_safe($json_data->item_seller_get_response->item->stuff_status);
...
// 	$product_quality = "Used";
// }
// $sku = $json_data->item_seller_get_response->item->skus->sku;
...
// 		$prop 	= explode(":",$prop);
// 		$number = replace_safe($num_iid.$prop[1].$prop[5]);
// 		$prop[2] = replace_safe($prop[2]);
// 		$prop[3] = replace_safe($prop[3]);
// 		$prop[6] = replace_safe($prop[6]);
// 		$prop[7] = replace_safe($prop[7]);
// 		if($number == $format_list){
// 			$sql = "UPDATE product_related_info SET name='$title', format_list='$number', get_date=NOW() WHERE company_id='$company_id' AND product_id='$product_id' AND info_id='$num_iid'";
// 			mysql_query($sql,$_mysql_link_);
