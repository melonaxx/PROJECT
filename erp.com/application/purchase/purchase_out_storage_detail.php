...
if(!empty($_GET['id'])){
	$id = intval($_GET['id']);
	$sql = "SELECT m.id,m.number,m.total,m.price,m.store_id,m.staff_id,m.brief,m.supplier_id,m.status_receipt,m.status_refund,m.status_audit,m.action_date,m.body,f.freight_side,f.freight_amount,m.shipping_company,m.waybill_number,f.pay_method,m.total_way,m.total_finish,m.total_refund FROM purchase_main_info AS m LEFT JOIN finance_purchase AS f ON m.id=f.purchase_id WHERE m.company_id = '$company_id' AND m.id = '$id' ";
	...
	$sql = "SELECT id,product_id,format,parts_id,total,price,content,total_finish,total_refund,total_way FROM purchase_product WHERE company_id = '$company_id' AND purchase_id = '$id' ";
	$result = mysql_query($sql,$_mysql_link_);
	$no = 1;
	while($dbRow = mysql_fetch_object($result)){
		$product 				= array();
...
if(!empty($_POST['made']) && $_POST['zongshu_rk']>0){
	...
	for($i = 0;$i<count($product_id);$i++){
		...
		$total_refund[$i] 	= intval($out_sum[$i])+intval($out_sum2[$i]);
		$sql = "UPDATE purchase_product SET total_finish = '$total_finish[$i]',total_way = '$total_way[$i]', total_refund = '$total_refund[$i]' WHERE company_id = '$company_id' AND purchase_id = '$id' AND product_id = '$product_id[$i]'";
		mysql_query($sql,$_mysql_link_);
	}
	//判断采购商品退货状态
	...
	while($dbRow = mysql_fetch_object($result)){
		...
		if(intval($res['total_refund']) > 0 && intval($res['total_refund']) < intval($res['total'])){
			$sql = "UPDATE purchase_main_info SET status_refund = 'P' WHERE id = '{$res["id"]}' ";
			mysql_query($sql,$_mysql_link_);
		}
		if(intval($res['total_refund']) == intval($res['total'])){
			$sql = "UPDATE purchase_main_info SET status_refund = 'Y' WHERE id = '{$res["id"]}' ";
			mysql_query($sql,$_mysql_link_);
		}
	}
	//添加出库单和出库商品
	$sql = "INSERT INTO store_input_info SET company_id = '$company_id', `number`='$number_rk', store_type = 'Output', supplier_id = '$supplier_id', store_id = '$store_id',action_date = NOW(), total = '$zongshu_rk', price = '$zongjia_rk', purchase_id = '$number',input_staff_id = '$staff_id' ";
	$result = mysql_query($sql,$_mysql_link_);
	if(mysql_affected_rows($_mysql_link_) == 1){
		...
		for($i=0;$i<count($product_id_rk);$i++){
			...
			$parts_id[$i] = replace_safe($parts_id_rk[$i]);
			$sum[$i] = intval($in_sum_rk[$i]);
			$pri[$i] = floatval($price_rk[$i]);
			$bod[$i] = replace_safe($rk_body[$i]);
			$pay[$i] = floatval($xiaoji[$i]);
			if($sum[$i] > 0){
				$sql = "INSERT INTO store_input_product SET info_id = '$info_id', product_id = '$pro[$i]', format = '$format[$i]', parts_id = '$parts_id[$i]', total = '$sum[$i]', price = '$pri[$i]',body = '$bod[$i]',payment = '$pay[$i]'";
				mysql_query($sql,$_mysql_link_);

				$sql = "UPDATE store_product SET total_real = total_real-'$sum[$i]',total_available=total_available-'$sum[$i]' WHERE company_id = '$company_id' AND store_id = '$store_id' AND product_id = '$pro[$i]' ";
				mysql_query($sql,$_mysql_link_);
			}
		}
	}
	...
	header('location:/purchase/purchase_out_storage_detail.php');
}
//修改采购摘要
...
{
	header("Content-content: text/html; charset=UTF-8");
