...
$status 		= array('N'=>'未付款','D'=>'部分付款','Y'=>'完成付款');

if(!empty($_REQUEST['id'])){
	$id = intval($_REQUEST['id']);
	$sql = "SELECT m.id,m.number,m.total,m.price,m.store_id,m.staff_id,m.brief,m.supplier_id,m.status_audit,m.status_refund,m.status_receipt,m.action_date,m.body,m.shipping_company,m.waybill_number,m.total_finish,m.total_way,m.total_refund,f.freight_side,f.freight_amount,f.pay_method,f.payment_already,f.payment_remain,f.status,f.payment_return FROM purchase_main_info AS m LEFT JOIN finance_purchase AS f ON m.id=f.purchase_id WHERE m.company_id = '$company_id' AND m.id = '$id' ";
	$result = mysql_query($sql,$_mysql_link_);
	while($dbRow = mysql_fetch_object($result)){
		$purchase 					= array();
...
if(!empty($_POST['made']) && $_POST['money']>0)
{
	...
	//判断客户类型，订金和欠款尾款为供应商，运费为快递公司
	$company_type = '';
	if($subject=='运费')
	{
		$company_type = 'Express';
	}else if($subject=='欠款尾款' || $subject=='订金'){
		$company_type = 'Supplier';
	}
	//添加资金流水记录
	$sql = "INSERT INTO finance_cash_logs SET company_id='$company_id',info_type='Purchase',info_id='$purchase_id',amount_date= NOW(),business_date= NOW(),type='Output',subject='$subject',company_type='$company_type',company_name='$supplier',`money`='$money',bank_id='$bank_id',body='$body' ";
	mysql_query($sql,$_mysql_link_);

	//修改采购已付金额
	$sql = "UPDATE finance_purchase SET payment_already=payment_already+'$money',payment_remain=payment_remain-'$money' WHERE company_id='$company_id' AND purchase_id='$purchase_id' ";
	mysql_query($sql,$_mysql_link_);

	//查询欠款金额,采购单总价
	$sql = "SELECT payment_total,payment_remain FROM finance_purchase WHERE company_id='$company_id' AND purchase_id='$purchase_id' ";
	$result = mysql_query($sql,$_mysql_link_);
	$sum 	= mysql_fetch_object($result);
	$payment_total 		= $sum->payment_total;
	$payment_remain 	= $sum->payment_remain;
	if($payment_remain == 0)
	{
		$sql = "UPDATE finance_purchase SET status = 'Y' WHERE company_id='$company_id' AND purchase_id='$purchase_id' ";
		mysql_query($sql,$_mysql_link_);
	}else if($payment_remain > 0 && $payment_remain < $payment_total){
		$sql = "UPDATE finance_purchase SET status = 'D' WHERE company_id='$company_id' AND purchase_id='$purchase_id' ";
		mysql_query($sql,$_mysql_link_);
	}else if($payment_remain == $payment_total){
		$sql = "UPDATE finance_purchase SET status = 'N' WHERE company_id='$company_id' AND purchase_id='$purchase_id' ";
		mysql_query($sql,$_mysql_link_);
	}
	//修改账户金额
	...
	header('location:/purchase/purchase_payment_detail.php');
}
