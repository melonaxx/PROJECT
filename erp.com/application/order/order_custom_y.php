...
include "../function.php";
	...
	if(isset($_POST['dz'])){
		...
		for($i=0;$i<count($id);$i++){
			$sql = "UPDATE order_custom SET status ='R' WHERE order_id='$id[$i]'";
	...
	if(!empty($_GET['find']))
	{
		...
		if(!empty($find))
		{
			...
			$main['find']	= $find;
			$page_param		= array();
			$page_param['find']		= replace_safe($_GET['find'], 20, false, false);
		}
	}
	...
	$sql = "SELECT COUNT(*) as total FROM order_info
		...
		LEFT JOIN order_custom ON order_info.id = order_custom.order_id
