...
if(!empty($_GET)){
    ...
    $sql = "SELECT id,name,template_id,content,sign FROM crm_message_template WHERE company_id='$company_id' AND status='SUCCESS' AND type='Remind' ";
...
if(!empty($_POST)){
    ...
    for($i=0;$i<count($aa);$i++){
       if($message_remain == 0){
            $no++;
            //添加失败日志
            $sql = "INSERT INTO crm_message_logs SET company_id='$company_id',order_id='$dd[$i]',template_id='$id',type='Remind',content='{$tem['content']}',status='N',reason='余额不足',action_date='$date' ";
            ...
            if($tem['strategy'] == '无'){
                ...
                if($tol_1 >= 15){
                    ...
                    if((strtotime($date)-strtotime($action_date_1[14])) < 86400){
                        $no++;
                        //添加失败日志
                        $sql = "INSERT INTO crm_message_logs SET company_id='$company_id',order_id='$dd[$i]',template_id='$id',type='Remind',content='{$tem['content']}',status='N',reason='同一用户24小时发送不能超过15条',action_date='$date' ";
                        mysql_query($sql,$_mysql_link_);
                    }else{
                        ...
                        if($tol_2 >= 3){
                            if((strtotime($date)-strtotime($action_date_2[0])) < 120){
                                ...
                                $no++;
                                //添加失败日志
                                $sql = "INSERT INTO crm_message_logs SET company_id='$company_id',order_id='$dd[$i]',template_id='$id',type='Remind',content='{$tem['content']}',status='N',reason='同一用户同一内容每2分钟只能发送一条',action_date='$date' ";
                                mysql_query($sql,$_mysql_link_);
                            }else if((strtotime($date)-strtotime($action_date_2[2]))<86400){
                                //当前时间距离第三条信息发送时间不够24小时，不能发送；
                                $no++;
                                //添加失败日志
                                $sql = "INSERT INTO crm_message_logs SET company_id='$company_id',order_id='$dd[$i]',template_id='$id',type='Remind',content='{$tem['content']}',status='N',reason='同一用户同一内容24小时发送不能超过三条',action_date='$date' ";
                                mysql_query($sql,$_mysql_link_);
                            }else{
                                //可以发送，发送请求
                                //发送信息请求
                                ...
                                if($arr['code']==0){
                                    $yes++;
                                    //添加成功日志
                                    $sql = "INSERT INTO crm_message_logs SET company_id='$company_id',order_id='$dd[$i]',template_id='$id',type='Remind',content='{$tem['content']}',status='Y',action_date='$date' ";
                                    mysql_query($sql,$_mysql_link_);
                                    //剩余短信数减1
                                    $message_remain--;
                                    //获取下订单的店铺id
                                    ...
                                    $no++;
                                    //添加失败日志
                                    $sql = "INSERT INTO crm_message_logs SET company_id='$company_id',order_id='$dd[$i]',template_id='$id',type='Remind',content='{$tem['content']}',status='N',reason='{$arr['msg']}',action_date='$date' ";
                                    mysql_query($sql,$_mysql_link_);
                                }
                            }
                        }else{
                ...
                if($tem['strategy_status'] == 'N'){
                    ...
                    if($sta[0] == 'D'){
                        ...
                        if($tyeory_amount < $sta[1]){
                            $no++;
                            //添加失败日志
                            $sql = "INSERT INTO crm_message_logs SET company_id='$company_id',order_id='$dd[$i]',template_id='$id',type='Remind',content='{$tem['content']}',status='N',reason='不在发送规则范围',action_date='$date' ";
                            mysql_query($sql,$_mysql_link_);
                        }else{
                            //获取该条订单已发送信息的时间和总条数，按时间降序排列
                            $sql = "SELECT action_date FROM crm_message_logs WHERE  company_id = '$company_id' AND order_id = '$dd[$i]' AND status='Y' ORDER BY action_date desc ";
