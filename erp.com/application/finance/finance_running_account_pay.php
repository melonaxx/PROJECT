...
$GroupList	= array();
$group 		= array();
$sql = "SELECT id,name,level,sort,parent_id FROM finance_spending_topic WHERE company_id='$company_id' AND status='Y' ORDER BY parent_id, sort ";
...
if(!empty($_POST)){
	...
	$subject 		= replace_safe(trim($_POST['subject'],' '));
	...
	$sql = "INSERT INTO finance_cash_logs SET bank_id='$bank_id', company_id='$company_id',amount_date=NOW(), business_date='$buiness_date', type='$type', subject='$subject', company_type='$company_type', company_name='$company_name', `money`='$money', body='$body'";
