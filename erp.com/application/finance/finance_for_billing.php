...
$addon[]    = "order_receiver.tax_status='N'";
...
$sql = "SELECT COUNT(*) as total FROM order_receiver
	...
	LEFT JOIN order_delivery ON order_receiver.id = order_delivery.id
	 ".$where;
...
$sql = "SELECT
		...
		order_receiver.tax_type,order_receiver.tax_title,order_receiver.tax_text,order_delivery.delivery_status
		FROM  order_receiver
		LEFT JOIN order_source ON order_receiver.id = order_source.id
		LEFT JOIN order_delivery ON order_receiver.id = order_delivery.id
		 ".$where." LIMIT ".$limit;
