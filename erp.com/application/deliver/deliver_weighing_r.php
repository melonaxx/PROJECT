...
$company_id=$_SESSION['_application_info_']['company_id'];
	...
	$sql = "SELECT COUNT(*) as total FROM order_info
	    ...
	    LEFT JOIN order_delivery ON order_info.id = order_delivery.id ".$where;
	...
	if($main['total'] > 0)
	{
		...
		FROM  order_info
		LEFT JOIN order_source ON order_info.id = order_source.id
		LEFT JOIN order_delivery ON order_info.id = order_delivery.id ".$where." ORDER BY order_info.id DESC LIMIT ".$limit;
		$result	= mysql_query($sql, $_mysql_link_);
		while($StoreInfo = mysql_fetch_object($result))
		{
			...
			while($res = mysql_fetch_object($result1)){
				...
				while($store = mysql_fetch_object($result6)){
					$list_audit['store_id']		    = $store->name;
				}
				$list_audit['total']				= $res->total;
			}

			$sql     = "SELECT express_id,number FROM order_express_paper WHERE company_id='$company_id' AND order_id='$StoreInfo->id'";
			...
			while($res = mysql_fetch_object($result2)){
				...
				while($store = mysql_fetch_object($result3)){
			...
			$sql = "SELECT theory_amount FROM finance_order WHERE order_id = '$StoreInfo->id' AND company_id='$company_id'";
			$result5 = mysql_query($sql,$_mysql_link_);
			while($store = mysql_fetch_object($result5)){
				$list_audit['theory_amount']  = $store->theory_amount;
			}
