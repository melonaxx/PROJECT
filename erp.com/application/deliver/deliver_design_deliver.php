...
while($rows2 = mysql_fetch_object($result2)){
	...
	$deliver_template['id']  	=  $rows2->id;
	$deliver_template['name'] =  $rows2->name;
...
$sql1 		= "SELECT name,image,print_orient,page_top,page_left,page_width,page_height,border FROM company_deliver_template_info WHERE company_id='$company_id' AND id='$template_id'";
...
while($rows1 = mysql_fetch_object($result1)){
	...
	$page_info['border']=$page_info['border']?$page_info['border']:1;
...
while($rows = mysql_fetch_object($result)){
	...
	if($rows->type =='other'){
		$deliver['id'] 				= $rows->id;
		$deliver['item_id'] 		= $rows->item_id;
		$deliver['item_top'] 		= $rows->item_top;
		$deliver['item_left']	 	= $rows->item_left;
		$deliver['item_width'] 		= $rows->item_width;
		$deliver['item_height'] 	= $rows->item_height;
		$deliver['item_font_size']	= $rows->item_font_size;
		$deliver['item_font_bold'] 	= $rows->item_font_bold;
		$deliver['name'] 			= $rows->name;
		$deliver['english'] 		= $rows->english;
		$deliver['item_type'] 		= $rows->type;
		$xtpl->assign("deliver",$deliver);
		$xtpl->parse("main.deliver");
