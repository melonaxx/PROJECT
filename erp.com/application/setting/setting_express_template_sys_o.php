...
if($_GET['action'] == 'add'){
	...
	$sql= "SELECT i.id,i.name,i.image,i.print_orient,i.paper_width,i.paper_height,i.paper_left,i.paper_top,i.border FROM main_order_template_info AS i WHERE i.id={$_GET['id']}";
	...
	$name = '配货模板'.rand(10,99);
	$sql2 = "INSERT into company_order_template_info SET 
	...
	$sql3 = "SELECT i.template_id,i.item_id,i.item_width,i.item_height,i.item_left,i.item_top,i.item_font_size,i.item_font_bold FROM main_order_template_position as i WHERE i.template_id={$_GET['id']}";
	$msq3 = mysql_query($sql3, $_mysql_link_);//echo $sql3;
		while($expressPosition = mysql_fetch_object($msq3)){
			...
			$sql4[] = "INSERT into company_order_template_position SET 
			company_id = '$company_id',
			template_id = '{$I}',
			item_id = '{$item_id}',
...
$sql	= "SELECT COUNT(*) as total FROM main_order_template_info $addon";
...
$sqll= "SELECT i.id,i.name,i.image,i.pub_date,i.print_orient,i.paper_width,i.paper_height,i.paper_left,i.paper_top,i.border FROM  main_order_template_info as i order by i.id DESC LIMIT ".$limit;	
