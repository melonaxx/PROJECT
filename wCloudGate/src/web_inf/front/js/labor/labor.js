$(function($) {
    ...
    function disSuccess(data){
        if(data != ""){
            ...
            for(var i=0;i<data.length;i++){
                if(data[i]['userid'] != 0){var dis = "<a class='unbundling' attr='"+data[i]['userid']+"'>解绑</a>";var sta= "已绑定:"+data[i]['name']}else{var dis = "<input class='checktd' type='checkbox'/>";var sta = "";}
                str += "<tr>";
                str += "<td>"+dis+"</td>";
                str += "<td>"+(i+1)+"</td>";
    ...
    $(document).on("click",".unbundling",function(){
        var userid = $(this).attr('attr');
        var seqno  = $(this).parent().next().next().html();
        var data = {userid:userid,seqno:seqno};
        util.ajax_post("/ubrelation.php",data,ubSuccess,ubFail);
    });
    function ubSuccess(data){
        window.location.reload();
    }
    function ubFail(data){
        alert("操作失败");
    }
...
//    });
    ...
    $(".empnum").on("click",function(){
        $(".laborid").val($(this).parent().attr('laborid'));
